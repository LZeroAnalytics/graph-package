...
export function handleBlock(params: TxRecords): void {
  ...
  for (let i = 0; i < params.records.length; i++) {
    ...
    const rawTx = rec.raw;
    if (!rawTx) {
      log.warning("Transaction {} missing raw data, skipping", [rec.id.toHexString()]);
      continue;
    }
    ...
    tx.from = rawTx.from as Bytes;
    
    if (rawTx.to) {
      tx.to = rawTx.to as Bytes;
    } else {
      tx.to = null;
    }
    
    if (rawTx.value) {
      tx.value = safeParseBigInt(rawTx.value, BigInt.fromI32(0));
    } else {
      tx.value = BigInt.fromI32(0);
    }
    
    if (rawTx.gas_limit) {
      tx.gasLimit = safeParseBigInt(rawTx.gas_limit, BigInt.fromI32(21000));
    } else {
      tx.gasLimit = BigInt.fromI32(21000);
    }
    
    if (rawTx.gas_price) {
      tx.gasPrice = safeParseOptionalBigInt(rawTx.gas_price);
    } else {
      tx.gasPrice = null;
    }
    
    if (rawTx.max_fee_per_gas) {
      tx.maxFeePerGas = safeParseOptionalBigInt(rawTx.max_fee_per_gas);
    } else {
      tx.maxFeePerGas = null;
    }
    
    if (rawTx.max_priority_fee_per_gas) {
      tx.maxPriorityFeePerGas = safeParseOptionalBigInt(rawTx.max_priority_fee_per_gas);
    } else {
      tx.maxPriorityFeePerGas = null;
    }
    
    if (rawTx.access_list) {
      tx.accessList = rawTx.access_list;
    } else {
    ...
    const decodedTx = rec.decoded;
    if (decodedTx && decodedTx.fn_sig && decodedTx.args_json && decodedTx.abi_source) {
      tx.fnSig = decodedTx.fn_sig;
      tx.args = decodedTx.args_json;
      tx.abiSource = decodedTx.abi_source;
    } else if (rawTx.data && rawTx.data.length > 2 && rawTx.to) {
      const hexString = rawTx.data.toHexString();
